...
| data | object | 必须 | 返回的数据 |
| \|- total | number | 必须 | 总记录数 |
| \|- rows | object[] | 必须 | 数据列表item 类型: object |
| \|- \|- id | number | 非必须 | id |
...
| beginDate | string | 必须 | 开课时间 |
| endDate | string | 必须 | 结课时间 |
| masterId | number | 非必须 | 班主任ID(员工ID) |
| subject | number | 必须 | 学科, 1:java, 2:前端, 3:大数据, 4:Python, 5:Go, 6:嵌入式 |
...
```json
{
  ...
  "data": {
    ...
    "username": "songjiang",
    "name": "宋江",
    "token": "eyJhbGciOiJIUzI1NiJ9.eyJpZCI6MiwidXNlcm5hbWUiOiJzb25namlhbmciLCJleHAiOjE2OTg3MDE3NjJ9.w06EkRXTep6SrvMns3w5RKe79nxauDe7fdMhBLK6MKY"
  }
}
```
